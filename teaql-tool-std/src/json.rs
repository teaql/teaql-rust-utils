...
impl JsonTool {
    ...
    pub fn parse(&self, s: &str) -> Result<Value> {
        from_str(s).map_err(|e| TeaQLToolError::ParseError(e.to_string()))
    }

    pub fn stringify(&self, v: &Value) -> Result<String> {
        serde_json::to_string(v).map_err(|e| TeaQLToolError::ParseError(e.to_string()))
    }

    pub fn stringify_pretty(&self, v: &Value) -> Result<String> {
        serde_json::to_string_pretty(v).map_err(|e| TeaQLToolError::ParseError(e.to_string()))
    }
    ...
    pub fn patch(&self, v: &mut Value, p: &Patch) -> Result<()> {
        patch(v, p).map_err(|e| TeaQLToolError::ExecutionError(e.to_string()))
    }
}
